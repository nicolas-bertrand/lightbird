...
const Request   &Engine::getRequestObject() const
{
    return (this->request);
}
...
bool        Engine::_onSerialize(LightBird::IOnSerialize::Serialize type)
{
    ...
    QMapIterator<QString, LightBird::IOnSerialize *> it(Plugins::instance()->getInstances<LightBird::IOnSerialize>(this->client.getValidator()));
    while (it.hasNext())
    {
