...
LIBS += \
    ...
    -L../../../../../server/plugins/Extensions/ImageMagick/Library/lib
...
linux {
    #./configure --prefix=PATH --with-magick-plus-plus=yes --with-jpeg=yes --with-png=yes --with-tiff=yes --enable-shared=no --without-perl --with-rsvg=yes --with-tiff=yes --with-webp=yes --with-openjp2=yes CXXFLAGS="-fPIC" CFLAGS="-fPIC"
    QMAKE_LFLAGS += -z defs
    LIBS += -lMagick++ -lMagickCore -lMagickWand -ljpeg -lpng -ltiff -lwebp -ljbig -lfreetype -lpng12 -lfontconfig -lXext -lSM -lICE -lX11 -llzma -lrsvg-2 -lm -lgio-2.0 -lgdk_pixbuf-2.0 -lcairo -lgobject-2.0 -lglib-2.0 -lz -lgomp
    DEFINES += MAGICKCORE_QUANTUM_DEPTH=16 MAGICKCORE_HDRI_ENABLE=0
}
