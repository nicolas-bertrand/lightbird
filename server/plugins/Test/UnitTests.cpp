...
bool            UnitTests::_accounts()
{
    ...
    try
    {
        ...
        ASSERT(a2->setIdFromIdentifiantAndSalt(this->api.sha256("a3" + this->api.sha256("p1" + a2->getIdFromName("a3").toAscii()) + "salt").data(), "salt"));
        ASSERT(a2->setIdFromIdentifiantAndSalt(this->api.sha256(QByteArray("a3") + a1->getPassword().toAscii() + "salt").data(), "salt"));
        ASSERT(a2->setIdFromIdentifiantAndSalt(this->api.sha256(QByteArray("a2") + "salt").data(), "salt"));
        ASSERT(a1->getPassword() == this->api.sha256("p1" + a2->getIdFromName("a3").toAscii()));
        ASSERT(a1->setPassword(""));
        ASSERT(a1->getPassword().isEmpty());
        ASSERT(a1->setPassword("p2"));
        ASSERT(a1->getPassword() == this->api.sha256("p2" + a2->getIdFromName("a3").toAscii()));
        ASSERT(a1->isActive());
...
bool            UnitTests::_groups()
{
    ...
    try
    {
        ASSERT(g1->add("b1"));
        ASSERT(!g1->add("b1", ""));
        ASSERT(g1->getTableId() == LightBird::ITable::Groups);
